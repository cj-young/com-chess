...
export default function movesToFEN(moves: Move[]) {
  ...
  // Find last pawn move *** Only looks at pawns that are still active. If the
  // last pawn is no longer active, then it must have been captured, in which
  // case the last capture will be used instead anyway
...
function findLastActivePawnMove(pieces: Piece[], moves: Move[]) {
...
function findLastCapture(moves: Move[]) {
  const board: any[] = Array.from({ length: 64 }, () => null);
  ...
  let enPassantSquare = null;
  let lastCapture = 0;
  for (let i = 0; i < indexMoves.length; i++) {
    const move = indexMoves[i];
